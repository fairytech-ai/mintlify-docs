...
| 플레이스홀더 | 입력할 값 |
| :-- | :-- |
| `{SDK_ARTIFACT_ID}` | 연동 구성에 따라 Fairy에서 전달한 SDK artifact ID. 예: `moment-x`, `moment-x-push` |
| `{SDK_VERSION}` | Fairy에서 전달한 SDK 버전. 예: `2.0.2` |
| `{MAVEN_SECRET}` | Fairy에서 전달한 Maven Secret Key |
...
<Note>
  `{MAVEN_SECRET}`에는 사전에 전달드린 Maven Secret Key를 입력합니다.
</Note>
...
repositories {
    maven {
        ...
        // Fairy에서 전달한 Maven Secret Key를 입력합니다.
        credentials {
...
<Note>
  `{PROJECT_ID}`, `{SDK_API_KEY}`에는 사전에 전달드린 값을 입력합니다.
</Note>
