...
아래 항목을 `eng@fairytech.ai`로 전달해 주시면, 프로젝트 설정 후에 고객사에 맞는 SDK 설치값을 회신드립니다.
...
<Note>
  개발용 인증서는 로컬 환경마다 다른 `debug.keystore`를 사용할 수 있으므로 SHA-1 키해시도 달라질 수 있습니다. 개발에 사용하는 각 로컬 환경의 SHA-1을 모두 전달해 주세요. 배포용은 실제 앱 서명 인증서의 SHA-1을 사용합니다. Google Play 앱 서명을 사용한다면 업로드 키뿐 아니라 Play Console의 앱 서명 키 SHA-1도 확인해 주세요.
</Note>
...
출력에서 대상 variant의 `SHA1`을 확인하고, 해당 앱 패키지명과 함께 전달해 주세요. 자세한 내용은 [Google의 클라이언트 인증 가이드](https://developers.google.com/android/guides/client-auth)를 참고하세요.
...
<Note>
  `{MAVEN_SECRET}`에는 사전에 제공한 Maven Secret Key를 입력합니다.
</Note>
...
<Note>
  `{PROJECT_ID}`, `{SDK_API_KEY}`에는 사전에 제공한 값을 입력합니다.
</Note>
